...
const message = {
  ...
  getByBookAndTwoPeople: function(id1, id2, idbook, callback) {
    // get all the messages of a specified book between 2 and only 2 specified people
    return db.query(
      "select b.idbook, b.image as book_image, b.idmember as book_idmember, m.idmember, m.idreceiver, m.message, m.time, mb.image as member_image, concat(mb.firstname, ' ', mb.lastname) as sender_name, concat(mb2.firstname, ' ', mb2.lastname) as receiver_name from book b left join message m on b.idbook=m.idbook inner join `member` mb on mb.idmember=m.idmember inner join `member` mb2 on mb2.idmember=m.idreceiver where b.idbook=? and (m.idmember=? or m.idreceiver=?) and (m.idmember=? or m.idreceiver=?) order by time asc",
